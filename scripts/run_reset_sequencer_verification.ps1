...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for reset_sequencer verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\clocking_reset_power_and_timing\reset_sequencer'
...
$simVvp = Join-Path $simOutRoot 'reset_sequencer_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'reset_sequencer_formal'

Run-Step -Name 'reset_sequencer simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'reset_sequencer.sv'),
    (Join-Path $simRoot 'reset_sequencer_tb.sv')
)

Run-Step -Name 'reset_sequencer simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'reset_sequencer yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'reset_sequencer.sv') + '; hierarchy -check -top reset_sequencer; proc; opt; stat')
)

Run-Step -Name 'reset_sequencer formal check' -Command $sby -Arguments @(
    '-f',
    'reset_sequencer_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('reset_sequencer verification complete. Outputs are in ' + $outRoot)
