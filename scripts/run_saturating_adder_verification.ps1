...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for saturating_adder verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\arithmetic_and_numeric_computing\saturating_adder'
...
$simVvp = Join-Path $simOutRoot 'saturating_adder_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'saturating_adder_formal'

Run-Step -Name 'saturating_adder simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'saturating_adder.sv'),
    (Join-Path $simRoot 'saturating_adder_tb.sv')
)

Run-Step -Name 'saturating_adder simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'saturating_adder yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'saturating_adder.sv') + '; hierarchy -check -top saturating_adder; proc; opt; stat')
)

Run-Step -Name 'saturating_adder formal check' -Command $sby -Arguments @(
    '-f',
    'saturating_adder_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('saturating_adder verification complete. Outputs are in ' + $outRoot)
