...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for wakeup_controller verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\clocking_reset_power_and_timing\wakeup_controller'
...
$simVvp = Join-Path $simOutRoot 'wakeup_controller_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'wakeup_controller_formal'

Run-Step -Name 'wakeup_controller simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'wakeup_controller.sv'),
    (Join-Path $simRoot 'wakeup_controller_tb.sv')
)

Run-Step -Name 'wakeup_controller simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'wakeup_controller yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'wakeup_controller.sv') + '; hierarchy -check -top wakeup_controller; proc; opt; stat')
)

Run-Step -Name 'wakeup_controller formal check' -Command $sby -Arguments @(
    '-f',
    'wakeup_controller_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('wakeup_controller verification complete. Outputs are in ' + $outRoot)
