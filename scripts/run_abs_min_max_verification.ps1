...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for abs_min_max verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\arithmetic_and_numeric_computing\abs_min_max'
...
$simVvp = Join-Path $simOutRoot 'abs_min_max_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'abs_min_max_formal'

Run-Step -Name 'abs_min_max simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'abs_min_max.sv'),
    (Join-Path $simRoot 'abs_min_max_tb.sv')
)

Run-Step -Name 'abs_min_max simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'abs_min_max yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'abs_min_max.sv') + '; hierarchy -check -top abs_min_max; proc; opt; stat')
)

Run-Step -Name 'abs_min_max formal check' -Command $sby -Arguments @(
    '-f',
    'abs_min_max_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('abs_min_max verification complete. Outputs are in ' + $outRoot)
