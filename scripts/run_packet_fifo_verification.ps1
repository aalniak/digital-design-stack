...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for packet_fifo verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\packet_fifo'
$streamFifoRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\stream_fifo\rtl'
...
$simVvp = Join-Path $simOutRoot 'packet_fifo_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'packet_fifo_formal'

Run-Step -Name 'packet_fifo simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $streamFifoRoot 'stream_fifo.sv'),
    (Join-Path $rtlRoot 'packet_fifo.sv'),
    (Join-Path $simRoot 'packet_fifo_tb.sv')
)

Run-Step -Name 'packet_fifo simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'packet_fifo yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $streamFifoRoot 'stream_fifo.sv') + ' ' + (Join-Path $rtlRoot 'packet_fifo.sv') + '; hierarchy -check -top packet_fifo; proc; opt; stat')
)

Run-Step -Name 'packet_fifo formal check' -Command $sby -Arguments @(
    '-f',
    'packet_fifo_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('packet_fifo verification complete. Outputs are in ' + $outRoot)
