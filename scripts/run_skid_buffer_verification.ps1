...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for skid_buffer verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\skid_buffer'
...
$simVvp = Join-Path $simOutRoot 'skid_buffer_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'skid_buffer_formal'

Run-Step -Name 'skid_buffer simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'skid_buffer.sv'),
    (Join-Path $simRoot 'skid_buffer_tb.sv')
)

Run-Step -Name 'skid_buffer simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'skid_buffer yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'skid_buffer.sv') + '; hierarchy -check -top skid_buffer; proc; opt; stat')
)

Run-Step -Name 'skid_buffer formal proof' -Command $sby -Arguments @(
    '-f',
    'skid_buffer_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('skid_buffer verification complete. Outputs are in ' + $outRoot)
