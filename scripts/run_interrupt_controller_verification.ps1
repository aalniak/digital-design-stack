...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for interrupt_controller verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\interrupt_controller'
...
$simVvp = Join-Path $simOutRoot 'interrupt_controller_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'interrupt_controller_formal'

Run-Step -Name 'interrupt_controller simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'interrupt_controller.sv'),
    (Join-Path $simRoot 'interrupt_controller_tb.sv')
)

Run-Step -Name 'interrupt_controller simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'interrupt_controller yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'interrupt_controller.sv') + '; hierarchy -check -top interrupt_controller; proc; opt; stat')
)

Run-Step -Name 'interrupt_controller formal proof' -Command $sby -Arguments @(
    '-f',
    'interrupt_controller_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('interrupt_controller verification complete. Outputs are in ' + $outRoot)
