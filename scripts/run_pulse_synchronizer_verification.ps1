...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for pulse_synchronizer verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\pulse_synchronizer'
...
$simVvp = Join-Path $simOutRoot 'pulse_synchronizer_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'pulse_synchronizer_formal'

Run-Step -Name 'pulse simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'pulse_synchronizer.sv'),
    (Join-Path $simRoot 'pulse_synchronizer_tb.sv')
)

Run-Step -Name 'pulse simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'pulse yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'pulse_synchronizer.sv') + '; hierarchy -check -top pulse_synchronizer; proc; opt; stat')
)

Run-Step -Name 'pulse formal proof' -Command $sby -Arguments @(
    '-f',
    'pulse_synchronizer_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('pulse_synchronizer verification complete. Outputs are in ' + $outRoot)
