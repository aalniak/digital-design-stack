...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for bus_synchronizer verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\bus_synchronizer'
...
$simVvp = Join-Path $simOutRoot 'bus_synchronizer_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'bus_synchronizer_formal'

Run-Step -Name 'bus simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'bus_synchronizer.sv'),
    (Join-Path $simRoot 'bus_synchronizer_tb.sv')
)

Run-Step -Name 'bus simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'bus yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'bus_synchronizer.sv') + '; hierarchy -check -top bus_synchronizer; proc; opt; stat')
)

Run-Step -Name 'bus formal proof' -Command $sby -Arguments @(
    '-f',
    'bus_synchronizer_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('bus_synchronizer verification complete. Outputs are in ' + $outRoot)
