...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for priority_arbiter verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\priority_arbiter'
...
$simVvp = Join-Path $simOutRoot 'priority_arbiter_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'priority_arbiter_formal'

Run-Step -Name 'priority_arbiter simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'priority_arbiter.sv'),
    (Join-Path $simRoot 'priority_arbiter_tb.sv')
)

Run-Step -Name 'priority_arbiter simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'priority_arbiter yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'priority_arbiter.sv') + '; hierarchy -check -top priority_arbiter; proc; opt; stat')
)

Run-Step -Name 'priority_arbiter formal proof' -Command $sby -Arguments @(
    '-f',
    'priority_arbiter_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('priority_arbiter verification complete. Outputs are in ' + $outRoot)
