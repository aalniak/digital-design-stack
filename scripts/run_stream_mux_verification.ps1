...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for stream_mux verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\stream_mux'
...
$simVvp = Join-Path $simOutRoot 'stream_mux_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'stream_mux_formal'

Run-Step -Name 'stream_mux simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'stream_mux.sv'),
    (Join-Path $simRoot 'stream_mux_tb.sv')
)

Run-Step -Name 'stream_mux simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'stream_mux yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'stream_mux.sv') + '; hierarchy -check -top stream_mux; proc; opt; stat')
)

Run-Step -Name 'stream_mux formal check' -Command $sby -Arguments @(
    '-f',
    'stream_mux_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('stream_mux verification complete. Outputs are in ' + $outRoot)
