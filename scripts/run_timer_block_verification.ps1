...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for timer_block verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\timer_block'
...
$simVvp = Join-Path $simOutRoot 'timer_block_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'timer_block_formal'

Run-Step -Name 'timer_block simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'timer_block.sv'),
    (Join-Path $simRoot 'timer_block_tb.sv')
)

Run-Step -Name 'timer_block simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'timer_block yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'timer_block.sv') + '; hierarchy -check -top timer_block; proc; opt; stat')
)

Run-Step -Name 'timer_block formal proof' -Command $sby -Arguments @(
    '-f',
    'timer_block_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('timer_block verification complete. Outputs are in ' + $outRoot)
