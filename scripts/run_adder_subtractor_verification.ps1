...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for adder_subtractor verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\arithmetic_and_numeric_computing\adder_subtractor'
...
$simVvp = Join-Path $simOutRoot 'adder_subtractor_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'adder_subtractor_formal'

Run-Step -Name 'adder_subtractor simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'adder_subtractor.sv'),
    (Join-Path $simRoot 'adder_subtractor_tb.sv')
)

Run-Step -Name 'adder_subtractor simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'adder_subtractor yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'adder_subtractor.sv') + '; hierarchy -check -top adder_subtractor; proc; opt; stat')
)

Run-Step -Name 'adder_subtractor formal check' -Command $sby -Arguments @(
    '-f',
    'adder_subtractor_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('adder_subtractor verification complete. Outputs are in ' + $outRoot)
