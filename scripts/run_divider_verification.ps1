...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for divider verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\arithmetic_and_numeric_computing\divider'
...
$simVvp = Join-Path $simOutRoot 'divider_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'divider_formal'

Run-Step -Name 'divider simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'divider.sv'),
    (Join-Path $simRoot 'divider_tb.sv')
)

Run-Step -Name 'divider simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'divider yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'divider.sv') + '; hierarchy -check -top divider; proc; opt; stat')
)

Run-Step -Name 'divider formal check' -Command $sby -Arguments @(
    '-f',
    'divider_formal.sby',
    '-d',
    $formalRunDir
...
Write-Output ('divider verification complete. Outputs are in ' + $outRoot)
