...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for retention_controller verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\clocking_reset_power_and_timing\retention_controller'
...
$simVvp = Join-Path $simOutRoot 'retention_controller_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'retention_controller_formal'

Run-Step -Name 'retention_controller simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'retention_controller.sv'),
    (Join-Path $simRoot 'retention_controller_tb.sv')
)

Run-Step -Name 'retention_controller simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'retention_controller yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'retention_controller.sv') + '; hierarchy -check -top retention_controller; proc; opt; stat')
)

Run-Step -Name 'retention_controller formal check' -Command $sby -Arguments @(
    '-f',
    'retention_controller_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('retention_controller verification complete. Outputs are in ' + $outRoot)
