...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for comparator_tree verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\arithmetic_and_numeric_computing\comparator_tree'
...
$simVvp = Join-Path $simOutRoot 'comparator_tree_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'comparator_tree_formal'

Run-Step -Name 'comparator_tree simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'comparator_tree.sv'),
    (Join-Path $simRoot 'comparator_tree_tb.sv')
)

Run-Step -Name 'comparator_tree simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'comparator_tree yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'comparator_tree.sv') + '; hierarchy -check -top comparator_tree; proc; opt; stat')
)

Run-Step -Name 'comparator_tree formal check' -Command $sby -Arguments @(
    '-f',
    'comparator_tree_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('comparator_tree verification complete. Outputs are in ' + $outRoot)
