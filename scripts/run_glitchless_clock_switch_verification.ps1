...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for glitchless_clock_switch verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\clocking_reset_power_and_timing\glitchless_clock_switch'
...
$simVvp = Join-Path $simOutRoot 'glitchless_clock_switch_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'glitchless_clock_switch_formal'

Run-Step -Name 'glitchless_clock_switch simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'glitchless_clock_switch.sv'),
    (Join-Path $simRoot 'glitchless_clock_switch_tb.sv')
)

Run-Step -Name 'glitchless_clock_switch simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'glitchless_clock_switch yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'glitchless_clock_switch.sv') + '; hierarchy -check -top glitchless_clock_switch; proc; opt; stat')
)

Run-Step -Name 'glitchless_clock_switch formal check' -Command $sby -Arguments @(
    '-f',
    'glitchless_clock_switch_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('glitchless_clock_switch verification complete. Outputs are in ' + $outRoot)
