...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for clock_fail_detector verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\clocking_reset_power_and_timing\clock_fail_detector'
...
$simVvp = Join-Path $simOutRoot 'clock_fail_detector_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'clock_fail_detector_formal'

Run-Step -Name 'clock_fail_detector simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'clock_fail_detector.sv'),
    (Join-Path $simRoot 'clock_fail_detector_tb.sv')
)

Run-Step -Name 'clock_fail_detector simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'clock_fail_detector yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'clock_fail_detector.sv') + '; hierarchy -check -top clock_fail_detector; proc; opt; stat')
)

Run-Step -Name 'clock_fail_detector formal check' -Command $sby -Arguments @(
    '-f',
    'clock_fail_detector_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('clock_fail_detector verification complete. Outputs are in ' + $outRoot)
