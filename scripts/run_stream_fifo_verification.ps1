...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for stream_fifo verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\stream_fifo'
...
$simVvp = Join-Path $simOutRoot 'stream_fifo_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'stream_fifo_formal'

Run-Step -Name 'stream_fifo simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'stream_fifo.sv'),
    (Join-Path $simRoot 'stream_fifo_tb.sv')
)

Run-Step -Name 'stream_fifo simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'stream_fifo yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'stream_fifo.sv') + '; hierarchy -check -top stream_fifo; proc; opt; stat')
)

Run-Step -Name 'stream_fifo formal check' -Command $sby -Arguments @(
    '-f',
    'stream_fifo_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('stream_fifo verification complete. Outputs are in ' + $outRoot)
