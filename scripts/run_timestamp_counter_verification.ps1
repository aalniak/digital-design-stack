...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for timestamp_counter verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\clocking_reset_power_and_timing\timestamp_counter'
...
$simVvp = Join-Path $simOutRoot 'timestamp_counter_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'timestamp_counter_formal'

Run-Step -Name 'timestamp_counter simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'timestamp_counter.sv'),
    (Join-Path $simRoot 'timestamp_counter_tb.sv')
)

Run-Step -Name 'timestamp_counter simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'timestamp_counter yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'timestamp_counter.sv') + '; hierarchy -check -top timestamp_counter; proc; opt; stat')
)

Run-Step -Name 'timestamp_counter formal check' -Command $sby -Arguments @(
    '-f',
    'timestamp_counter_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('timestamp_counter verification complete. Outputs are in ' + $outRoot)
