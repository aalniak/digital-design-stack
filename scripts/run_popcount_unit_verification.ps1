...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for popcount_unit verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\arithmetic_and_numeric_computing\popcount_unit'
...
$simVvp = Join-Path $simOutRoot 'popcount_unit_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'popcount_unit_formal'

Run-Step -Name 'popcount_unit simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'popcount_unit.sv'),
    (Join-Path $simRoot 'popcount_unit_tb.sv')
)

Run-Step -Name 'popcount_unit simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'popcount_unit yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'popcount_unit.sv') + '; hierarchy -check -top popcount_unit; proc; opt; stat')
)

Run-Step -Name 'popcount_unit formal check' -Command $sby -Arguments @(
    '-f',
    'popcount_unit_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('popcount_unit verification complete. Outputs are in ' + $outRoot)
