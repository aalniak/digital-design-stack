...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for clock_mux_controller verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\clocking_reset_power_and_timing\clock_mux_controller'
...
$simVvp = Join-Path $simOutRoot 'clock_mux_controller_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'clock_mux_controller_formal'

Run-Step -Name 'clock_mux_controller simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'clock_mux_controller.sv'),
    (Join-Path $simRoot 'clock_mux_controller_tb.sv')
)

Run-Step -Name 'clock_mux_controller simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'clock_mux_controller yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'clock_mux_controller.sv') + '; hierarchy -check -top clock_mux_controller; proc; opt; stat')
)

Run-Step -Name 'clock_mux_controller formal check' -Command $sby -Arguments @(
    '-f',
    'clock_mux_controller_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('clock_mux_controller verification complete. Outputs are in ' + $outRoot)
