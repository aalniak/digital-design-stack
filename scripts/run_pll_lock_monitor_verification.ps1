...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for pll_lock_monitor verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\clocking_reset_power_and_timing\pll_lock_monitor'
...
$simVvp = Join-Path $simOutRoot 'pll_lock_monitor_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'pll_lock_monitor_formal'

Run-Step -Name 'pll_lock_monitor simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'pll_lock_monitor.sv'),
    (Join-Path $simRoot 'pll_lock_monitor_tb.sv')
)

Run-Step -Name 'pll_lock_monitor simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'pll_lock_monitor yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'pll_lock_monitor.sv') + '; hierarchy -check -top pll_lock_monitor; proc; opt; stat')
)

Run-Step -Name 'pll_lock_monitor formal check' -Command $sby -Arguments @(
    '-f',
    'pll_lock_monitor_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('pll_lock_monitor verification complete. Outputs are in ' + $outRoot)
