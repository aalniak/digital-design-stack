...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for clock_divider verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\clocking_reset_power_and_timing\clock_divider'
...
$simVvp = Join-Path $simOutRoot 'clock_divider_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'clock_divider_formal'

Run-Step -Name 'clock_divider simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'clock_divider.sv'),
    (Join-Path $simRoot 'clock_divider_tb.sv')
)

Run-Step -Name 'clock_divider simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'clock_divider yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'clock_divider.sv') + '; hierarchy -check -top clock_divider; proc; opt; stat')
)

Run-Step -Name 'clock_divider formal check' -Command $sby -Arguments @(
    '-f',
    'clock_divider_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('clock_divider verification complete. Outputs are in ' + $outRoot)
