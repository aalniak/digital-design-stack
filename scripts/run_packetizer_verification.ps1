...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for packetizer verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\packetizer'
...
$simVvp = Join-Path $simOutRoot 'packetizer_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'packetizer_formal'

Run-Step -Name 'packetizer simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'packetizer.sv'),
    (Join-Path $simRoot 'packetizer_tb.sv')
)

Run-Step -Name 'packetizer simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'packetizer yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'packetizer.sv') + '; hierarchy -check -top packetizer; proc; opt; stat')
)

Run-Step -Name 'packetizer formal check' -Command $sby -Arguments @(
    '-f',
    'packetizer_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('packetizer verification complete. Outputs are in ' + $outRoot)
