...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for free_running_counter verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\free_running_counter'
...
$simVvp = Join-Path $simOutRoot 'free_running_counter_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'free_running_counter_formal'

Run-Step -Name 'free_running_counter simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'free_running_counter.sv'),
    (Join-Path $simRoot 'free_running_counter_tb.sv')
)

Run-Step -Name 'free_running_counter simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'free_running_counter yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'free_running_counter.sv') + '; hierarchy -check -top free_running_counter; proc; opt; stat')
)

Run-Step -Name 'free_running_counter formal proof' -Command $sby -Arguments @(
    '-f',
    'free_running_counter_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('free_running_counter verification complete. Outputs are in ' + $outRoot)
