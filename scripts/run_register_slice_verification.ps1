...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for register_slice verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\register_slice'
...
$simVvp = Join-Path $simOutRoot 'register_slice_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'register_slice_formal'

Run-Step -Name 'register_slice simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'register_slice.sv'),
    (Join-Path $simRoot 'register_slice_tb.sv')
)

Run-Step -Name 'register_slice simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'register_slice yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'register_slice.sv') + '; hierarchy -check -top register_slice; proc; opt; stat')
)

Run-Step -Name 'register_slice formal proof' -Command $sby -Arguments @(
    '-f',
    'register_slice_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('register_slice verification complete. Outputs are in ' + $outRoot)
