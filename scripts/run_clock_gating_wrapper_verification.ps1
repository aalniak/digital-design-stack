...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for clock_gating_wrapper verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\clocking_reset_power_and_timing\clock_gating_wrapper'
...
$simVvp = Join-Path $simOutRoot 'clock_gating_wrapper_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'clock_gating_wrapper_formal'

Run-Step -Name 'clock_gating_wrapper simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'clock_gating_wrapper.sv'),
    (Join-Path $simRoot 'clock_gating_wrapper_tb.sv')
)

Run-Step -Name 'clock_gating_wrapper simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'clock_gating_wrapper yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'clock_gating_wrapper.sv') + '; hierarchy -check -top clock_gating_wrapper; proc; opt; stat')
)

Run-Step -Name 'clock_gating_wrapper formal check' -Command $sby -Arguments @(
    '-f',
    'clock_gating_wrapper_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('clock_gating_wrapper verification complete. Outputs are in ' + $outRoot)
