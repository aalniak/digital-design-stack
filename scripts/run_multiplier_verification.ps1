...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for multiplier verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\arithmetic_and_numeric_computing\multiplier'
...
$simVvp = Join-Path $simOutRoot 'multiplier_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'multiplier_formal'

Run-Step -Name 'multiplier simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'multiplier.sv'),
    (Join-Path $simRoot 'multiplier_tb.sv')
)

Run-Step -Name 'multiplier simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'multiplier yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'multiplier.sv') + '; hierarchy -check -top multiplier; proc; opt; stat')
)

Run-Step -Name 'multiplier formal check' -Command $sby -Arguments @(
    '-f',
    'multiplier_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('multiplier verification complete. Outputs are in ' + $outRoot)
