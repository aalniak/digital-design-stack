...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for depacketizer verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\depacketizer'
...
$simVvp = Join-Path $simOutRoot 'depacketizer_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'depacketizer_formal'

Run-Step -Name 'depacketizer simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'depacketizer.sv'),
    (Join-Path $simRoot 'depacketizer_tb.sv')
)

Run-Step -Name 'depacketizer simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'depacketizer yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'depacketizer.sv') + '; hierarchy -check -top depacketizer; proc; opt; stat')
)

Run-Step -Name 'depacketizer formal check' -Command $sby -Arguments @(
    '-f',
    'depacketizer_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('depacketizer verification complete. Outputs are in ' + $outRoot)
