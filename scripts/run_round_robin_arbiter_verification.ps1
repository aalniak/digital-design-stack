...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for round_robin_arbiter verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\round_robin_arbiter'
...
$simVvp = Join-Path $simOutRoot 'round_robin_arbiter_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'round_robin_arbiter_formal'

Run-Step -Name 'round_robin_arbiter simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'round_robin_arbiter.sv'),
    (Join-Path $simRoot 'round_robin_arbiter_tb.sv')
)

Run-Step -Name 'round_robin_arbiter simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'round_robin_arbiter yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'round_robin_arbiter.sv') + '; hierarchy -check -top round_robin_arbiter; proc; opt; stat')
)

Run-Step -Name 'round_robin_arbiter formal proof' -Command $sby -Arguments @(
    '-f',
    'round_robin_arbiter_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('round_robin_arbiter verification complete. Outputs are in ' + $outRoot)
