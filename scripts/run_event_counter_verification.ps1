...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for event_counter verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\event_counter'
...
$simVvp = Join-Path $simOutRoot 'event_counter_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'event_counter_formal'

Run-Step -Name 'event_counter simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'event_counter.sv'),
    (Join-Path $simRoot 'event_counter_tb.sv')
)

Run-Step -Name 'event_counter simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'event_counter yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'event_counter.sv') + '; hierarchy -check -top event_counter; proc; opt; stat')
)

Run-Step -Name 'event_counter formal proof' -Command $sby -Arguments @(
    '-f',
    'event_counter_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('event_counter verification complete. Outputs are in ' + $outRoot)
