...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for reset_synchronizer verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\core_rtl_infrastructure_and_glue\reset_synchronizer'
...
$simVvp = Join-Path $simOutRoot 'reset_synchronizer_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'reset_synchronizer_formal'

Run-Step -Name 'reset simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'reset_synchronizer.sv'),
    (Join-Path $simRoot 'reset_synchronizer_tb.sv')
)

Run-Step -Name 'reset simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'reset yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'reset_synchronizer.sv') + '; hierarchy -check -top reset_synchronizer; proc; opt; stat')
)

Run-Step -Name 'reset formal proof' -Command $sby -Arguments @(
    '-f',
    'reset_synchronizer_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('reset_synchronizer verification complete. Outputs are in ' + $outRoot)
