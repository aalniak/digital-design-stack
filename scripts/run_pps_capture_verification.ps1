...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for pps_capture verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\clocking_reset_power_and_timing\pps_capture'
...
$simVvp = Join-Path $simOutRoot 'pps_capture_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'pps_capture_formal'

Run-Step -Name 'pps_capture simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'pps_capture.sv'),
    (Join-Path $simRoot 'pps_capture_tb.sv')
)

Run-Step -Name 'pps_capture simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'pps_capture yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'pps_capture.sv') + '; hierarchy -check -top pps_capture; proc; opt; stat')
)

Run-Step -Name 'pps_capture formal check' -Command $sby -Arguments @(
    '-f',
    'pps_capture_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('pps_capture verification complete. Outputs are in ' + $outRoot)
