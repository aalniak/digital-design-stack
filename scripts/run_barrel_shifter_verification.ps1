...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for barrel_shifter verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\arithmetic_and_numeric_computing\barrel_shifter'
...
$simVvp = Join-Path $simOutRoot 'barrel_shifter_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'barrel_shifter_formal'

Run-Step -Name 'barrel_shifter simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'barrel_shifter.sv'),
    (Join-Path $simRoot 'barrel_shifter_tb.sv')
)

Run-Step -Name 'barrel_shifter simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'barrel_shifter yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'barrel_shifter.sv') + '; hierarchy -check -top barrel_shifter; proc; opt; stat')
)

Run-Step -Name 'barrel_shifter formal check' -Command $sby -Arguments @(
    '-f',
    'barrel_shifter_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('barrel_shifter verification complete. Outputs are in ' + $outRoot)
