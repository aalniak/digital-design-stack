...
if (-not $suiteRoot) {
    throw 'Unable to locate OSS CAD Suite for complex_multiplier verification.'
}
...
$moduleRoot = Join-Path $workspaceRoot 'domains\arithmetic_and_numeric_computing\complex_multiplier'
...
$simVvp = Join-Path $simOutRoot 'complex_multiplier_tb.vvp'
$formalRunDir = Join-Path $formalOutRoot 'complex_multiplier_formal'

Run-Step -Name 'complex_multiplier simulation compile' -Command $iverilog -Arguments @(
    '-g2012',
    '-o',
    $simVvp,
    (Join-Path $rtlRoot 'complex_multiplier.sv'),
    (Join-Path $simRoot 'complex_multiplier_tb.sv')
)

Run-Step -Name 'complex_multiplier simulation run' -Command $vvp -Arguments @($simVvp)

Run-Step -Name 'complex_multiplier yosys synthesis sanity' -Command $yosys -Arguments @(
    '-p',
    ('read_verilog -sv ' + (Join-Path $rtlRoot 'complex_multiplier.sv') + '; hierarchy -check -top complex_multiplier; proc; opt; stat')
)

Run-Step -Name 'complex_multiplier formal check' -Command $sby -Arguments @(
    '-f',
    'complex_multiplier_formal.sby',
    '-d',
    $formalRunDir
) -WorkingDirectory $formalRoot

Write-Output ('complex_multiplier verification complete. Outputs are in ' + $outRoot)
